...
func TestGetMetrics_DaemonSetAlertname(t *testing.T) {
	srv := makePromServer(t, []PromResult{})
	defer srv.Close()

	prom := &PrometheusClient{HTTP: srv.Client(), URL: srv.URL}
	for _, name := range []string{"KubeDaemonSetNotScheduled", "KubeDaemonSetMisScheduled", "KubeDaemonSetRolloutStuck"} {
		alert := makeAlertWithLabels(map[string]string{"alertname": name})
		result := prom.GetMetrics(context.Background(), alert)
		if !strings.Contains(result, "DaemonSet Status") {
			t.Errorf("alert %q: expected DaemonSet Status section, got %q", name, result)
		}
	}
}
