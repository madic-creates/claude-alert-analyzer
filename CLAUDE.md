...
- **Metrics**: All metrics share the `alert_analyzer_*` prefix and carry a `product` ConstLabel (`k8s` | `checkmk`) applied at registry construction. Standard `go_*` and `process_*` collectors register on the same private registry via `WrapRegistererWith`. Served on `METRICS_PORT` (separate from the webhook port). Full metric reference: [`docs/observability.md`](docs/observability.md). Operator migration guide: [`docs/metrics-migration.md`](docs/metrics-migration.md).
